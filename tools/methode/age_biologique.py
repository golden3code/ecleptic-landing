...
(mesurée > VDOT en relèvement prudent > Nes/HUNT > Jackson/IMC), marge d'incertitude
de départ d'autant plus large que la source est indirecte, 7 modificateurs bornés et
signés (HRV, FC repos, taille, activité, sommeil asymétrique, nutrition, hygiène de vie
qui ne fait que vieillir), pas de double comptage avec l'équation, bornes, lissage EWMA.
Décision du user (24/09) : les chiffres du calcul (plafonds, marges, bornes, références
chiffrées) ne sont PAS publiés, ni ici ni dans les fiches qui renvoient à cette page ;
ils restent dans lib/bioAge. Le Readiness Score, lui, publie ses poids.
Volontairement non affirmé : « normes HUNT partout » (la référence neutre mêle
FRIEND et HUNT), le seuil nuit + repas + activité (retiré du code)
...
PAGE = {
    ...
    "date": "2026-09-24",
    "lead": "Ecleptic estime l'âge de ton corps en deux temps : ta VO₂max donne un <strong>âge de forme</strong>, puis sept facteurs tirés de tes habitudes l'ajustent, chacun dans des limites strictes. Voici la méthode, étape par étape, avec ses sources et ses garde-fous.",
    "body": """
...
<div class="figs">
  <div><span class="v">4</span><span class="u">sources de VO₂max, de la montre à l'IMC</span></div>
  <div><span class="v">7</span><span class="u">facteurs du quotidien, bornés et signés</span></div>
  <div><span class="v">J0</span><span class="u">une première estimation dès l'inscription</span></div>
</div>
...
<p>Plus la source est directe, plus le résultat est précis : la marge d'incertitude de ton âge biologique est la plus étroite quand ta montre mesure ta VO₂max, et elle s'élargit à mesure que la source devient indirecte.</p>
...
<p>L'âge de forme est ensuite ajusté par tes habitudes. Les facteurs n'ont pas tous le même poids, et chacun a son plafond, pour qu'aucun ne puisse à lui seul emporter le résultat :</p>
<ul>
<li><strong>La variabilité cardiaque.</strong> Ta <a href="/methode/variabilite-cardiaque.html">HRV</a> est comparée aux valeurs attendues pour ton âge : au-dessus, elle te rajeunit ; en dessous, elle te vieillit. Si tu prends un traitement qui ralentit le cœur, elle compte moins.</li>
<li><strong>La fréquence cardiaque au repos.</strong> Elle est située face à une valeur de référence : plus basse, elle te rajeunit ; plus haute, elle te vieillit.</li>
<li><strong>Le tour de taille.</strong> Il est comparé aux seuils de risque établis par l'OMS, qui diffèrent chez l'homme et chez la femme.</li>
<li><strong>L'activité.</strong> Tes minutes d'activité par semaine, face aux recommandations de l'OMS.</li>
<li><strong>Le sommeil.</strong> Ce facteur n'est pas symétrique : un sommeil déréglé peut te vieillir davantage qu'un bon sommeil ne peut te rajeunir. Des nuits de durée suffisante, sans excès, et des heures de coucher régulières jouent en ta faveur ; des nuits trop courtes ou trop longues et des couchers irréguliers pèsent dans l'autre sens. Le détail : <a href="/methode/besoin-de-sommeil.html">ton besoin de sommeil</a>.</li>
<li><strong>L'alimentation.</strong> La qualité de ce que tu manges, telle que la mesure ton <a href="/methode/nutrition.html">score nutrition</a>.</li>
<li><strong>L'hygiène de vie.</strong> Ce que tu déclares : la nicotine, et l'alcool selon le nombre de jours par semaine où tu bois. Ce facteur ne peut jamais te rajeunir.</li>
</ul>
...
<ul>
<li><strong>Des bornes :</strong> ton âge biologique ne peut pas s'écarter de ton âge réel au-delà d'une limite fixe, et il reste toujours dans une plage d'âges adultes réaliste. C'est un garde-fou contre les valeurs aberrantes.</li>
...
""",
    ...
    "faq": [
        ...
        {"q": "Peut-on connaître son âge biologique sans montre connectée ?",
         "a": "Oui. Sans VO₂max mesurée par une montre, Ecleptic l'estime sans effort avec l'équation HUNT (tour de taille, fréquence cardiaque au repos, niveau d'activité) ou celle de Jackson (IMC), et une course soutenue peut relever cette estimation grâce à la formule VDOT. Le résultat est simplement moins précis : sa marge d'incertitude de départ est plus large qu'avec une VO₂max mesurée par ta montre."},
        {"q": "Pourquoi mon âge biologique est-il plus élevé que mon âge réel ?",
         "a": "Soit ta VO₂max, l'ancre du calcul, est sous la valeur de référence pour ton âge et ton sexe, soit certains facteurs te vieillissent, comme des nuits trop courtes, trop longues ou irrégulières, une variabilité cardiaque basse pour ton âge, la nicotine ou l'alcool. Garde aussi en tête la marge d'incertitude, plus large quand ta VO₂max est estimée sans effort : un petit écart peut ne rien signifier, c'est la tendance sur plusieurs mois qui compte."},
    ],
